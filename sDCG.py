...
def sDCG(run_file_path, truth_xml_path, dd_info_path, bq=4, b=2, cutoff=10, verbose=False):
    ...
    if verbose:
        print(run_file_path)
        print('%8s' % 'topic-id', '%10s' % ('sDCG@' + str(cutoff)), '%10s' % ('nsDCG@' + str(cutoff))+'\t')
    ...
    for topic_id, topic_result in sorted_results:
        ...
        if verbose:
            print('%8s' % topic_id, '%10.7f' % sdcg, '%10.7f' % normalized_sdcg, '\t')

    if verbose:
        print('%8s' % 'all', '%10.7f' % statistics.mean(sdcg_list), '%10.7f' % statistics.mean(nsdcg_list),'\t')
